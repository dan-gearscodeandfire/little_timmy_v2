...
- **Interjection `!`/`?` are cap-EXEMPT (2026-08-19).** A `!` or `?` after a <=2-word fragment ("Ha!", "Oh man!", "Really?") is a beat, not a sentence end — `_is_real_terminator` returns False, so it neither burns the STRAIGHT 1-sentence budget (was: "Ha! The kettle is empty." spoken as just "Ha!") nor flushes as a clip boundary; the doubled-terminator "?!" degenerate solo-"!" clip is dead for the same reason. Scoped to `!?` ONLY — "No." still terminates. Rollback env `TIMMY_INTERJECTION_CAP_EXEMPT=0`. Gate/trim/splitters all share the predicate, so they cannot disagree. Piper prosody note: `!` raises the word ~35Hz and keeps the tail buoyant, `.` drops it ~an octave, `?` does NOTHING (skeletor_v1 has no interrogative prosody); `[scale=X]` one-shot length_scale plumbed through tts.speak/announce for canned lines.
- **`subscriber_hype` tool (2026-08-18): TERMINAL, pure-regex, PRE-classifier.** Dan announces a new subscriber ("we have a new subscriber!", "somebody just subscribed") -> randomized victory line from `prompts/subscriber_hype_lines.txt` (re-read live; text is EXACTLY what Piper speaks — dramatic beats are encoded as ` — — `, ellipses produce NO pause at length_scale 0.6, measured via `ops/measure_subscriber_lines.py`; lines support `||` segments, each with its own `[scale=X]`, stitched into ONE gapless clip by `tts.speak_segments` — per-word rate control). Detection in `conversation/subscriber_hype.py` is announcement-shaped with negation/question guards — bare "new subscriber" and "we don't have new subscribers" must NOT fire. Hook sits ABOVE the `classifier_enabled` gate in `tool_router.maybe_handle_tool_call` so it works with the classifier off; toggle `subscriber_hype_enabled` (default ON). False negatives acceptable, false positives are the bug.
...
- **Last edited:** 2026-08-19 by Claude (Fable 5), Dan in the loop — interjection cap-exemption in `reply_filter._is_real_terminator` (+5 tests); `length_scale` one-shot override through `_synthesize_raw`/`speak()`/`/api/announce`; subscriber_hype `[scale=X]` line directive.
- **Last edited:** 2026-08-18 by Claude (Fable 5), Dan in the loop — **subscriber-hype tool**: `conversation/subscriber_hype.py` + `prompts/subscriber_hype_lines.txt` (16 lines) + pre-classifier terminal hook in `tool_router` + `subscriber_hype_enabled` toggle (default ON) + `ops/measure_subscriber_lines.py` pause/duration harness. 40 new hermetic tests pass.
