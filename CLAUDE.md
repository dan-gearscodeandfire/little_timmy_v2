...
- ⚠**OPEN (enrollment FSM, out of scope 8-13):** bad STT ("Think of Dan.") let introductions latch **"Think" as a candidate name**, then it ate the NEXT unrelated turn and re-asked BYTE-IDENTICALLY twice. Canned dialog replies bypass the register/reply_filter path, so neither the sentence cap nor the self-imitation guard can see them. Live-observed 8-13.
