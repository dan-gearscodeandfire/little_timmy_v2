...
- **Verified against code on:** 2026-06-07 (`main`; vision-freshness Group C — **averted-gaze guard** for self-referential visual questions `f78731b` + **LT-OS frame-source** coupling `98e8c15`, deployed live, live in-frame test pending). Prior: 2026-06-06 (`main`; proactive-speech **barge-in guard** added — `capture.user_speaking`/`last_voice_ts` + `PROACTIVE_USER_SPEECH_GRACE_SEC`, supervisor issue #1, deployed live, live in-frame test pending). Prior: 2026-06-03 (`main`; weighted-RRF + coreference `d2af1e1`, proactive-speech + LT-OS toggle `696a961`, extraction queue/re-enqueue `31ed259`; vision localized scene-gate + speech-onset capture). 2026-05-30 (HEAD `5b435d3`).
